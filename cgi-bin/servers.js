...
// 블랙박스 서버 목록 CRUD
// 데이터: cgi-bin/servers/servers.json  (기존 위치 재사용)
//
//   GET    /cgi-bin/servers          전체 목록
//   GET    /cgi-bin/servers?alias=X  단건 조회
//   POST   /cgi-bin/servers          { alias, ip, port } 추가 (중복 alias → 409)
//   PUT    /cgi-bin/servers?alias=X  부분 수정
//   DELETE /cgi-bin/servers?alias=X  삭제
...
function parseBody() {
  // stdin 전체를 읽어서 JSON 파싱 (body 한 줄 이상일 수 있음)
  var lines = [];
  var line;
...
if (method === 'GET') {
  ...
  if (alias) {
    ...
    if (!found) reply(404, null, 'server not found: ' + alias);
    else reply(200, found);
  } else {
    reply(200, servers);
  }
  ...
  var body;
  try { body = parseBody(); } catch (e) { reply(400, null, 'invalid JSON: ' + e.message); throw e; }

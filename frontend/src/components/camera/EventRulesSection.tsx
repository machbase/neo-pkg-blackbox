...
export default function EventRulesSection({ cameraId, config, readOnly = false }: EventRulesSectionProps) {
    ...
    return (
        <article className="card">
                    ...
                    {rules.map((rule) => (
                        ...
                        >
                            <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                <div style={{ fontSize: 10, color: "var(--color-on-surface-disabled)", marginTop: 2 }}>{rule.record_mode}</div>
                            </div>
                            {!readOnly && (
            ...
            <EventRuleModal isOpen={modalOpen} onClose={handleModalClose} cameraId={cameraId} config={config} editRule={editingRule} ruleCount={rules.length} />
        </article>
    );
}

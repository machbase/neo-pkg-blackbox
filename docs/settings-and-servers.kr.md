...
- `Start At`
  ...
  - 화면에서는 사용자의 Local timezone 기준으로 입력합니다.
  - 입력한 값은 저장 시 내부적으로 UTC 기준으로 변환됩니다.
- `Interval Hours`
...
Manual Run이 끝나면 화면에 실행 결과가 표시됩니다. 작업 시작 시각인 `Started At`은 결과 영역의 우측 상단에 표시됩니다.
...
Manual Run은 실제 데이터와 파일을 정리할 수 있으므로, 운영 중에는 `Keep Hours`를 확인한 뒤 실행하는 것이 안전합니다.
